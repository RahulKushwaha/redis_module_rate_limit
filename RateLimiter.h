...
RateLimiterHandle createRateLimiter() {
  return (rk::projects::redis_rate_limiter::RateLimiter *) new rk::projects::redis_rate_limiter::RateLimiter();
}
...
bool consume(RateLimiterHandle handle, double toConsume) {
  return ((rk::projects::redis_rate_limiter::RateLimiter *) handle)->consume(toConsume);
}
